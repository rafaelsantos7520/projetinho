<x-layouts.app :title="'Admin da Loja'" :subtitle="'Nova Categoria'">
    ...
    <div class="max-w-7xl mx-auto">
        <div class="flex items-center justify-between gap-4 mb-6">
            <div>
                ...
                <p class="text-sm text-slate-500">Crie uma categoria e opcionalmente defina uma imagem.</p>
            </div>
        ...
        <form action="{{ route('tenant_admin.categories.store', ['tenant' => $tenantSlug]) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                    ...
                    <div class="lg:col-span-5">
                        <div class="bg-white border border-slate-200 rounded-2xl overflow-hidden">
                            <div class="p-5 border-b border-slate-100">
                                ...
                                <div class="text-xs text-slate-500 mt-1">Aparece na home como card de categoria.</div>
                            </div>
                            <div class="p-5 space-y-4">
                                <div class="rounded-2xl border border-slate-200 bg-slate-50 overflow-hidden">
                                    <div class="relative h-40 sm:h-48" data-cat-box>
                                        <img class="hidden h-full w-full object-cover" data-cat-preview />
                                        <div class="h-full w-full flex items-center justify-center text-slate-500 text-sm" data-cat-placeholder>Clique para enviar</div>
                                        <div class="hidden absolute inset-0 bg-black/30 items-center justify-center gap-2" data-cat-overlay>
                    ...
                    <div class="lg:col-span-7 space-y-6">
                        <div class="bg-white border border-slate-200 rounded-2xl overflow-hidden">
                            <div class="p-5 border-b border-slate-100">
                                ...
                                <div class="text-xs text-slate-500 mt-1">Nome e slug são gerados automaticamente.</div>
                            </div>
                            <div class="p-5 space-y-5">
                                <div>
                                    <label for="name" class="block text-sm font-medium text-slate-700 mb-1">Nome da Categoria</label>
                                    <input type="text" name="name" id="name" value="{{ old('name') }}" required class="w-full rounded-xl border border-slate-200 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-slate-900/20">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-end gap-4">
                            ...
                            <button type="submit" class="px-6 py-2.5 bg-slate-900 text-white font-medium rounded-xl hover:bg-slate-800 transition-colors">Salvar Categoria</button>
                        </div>
                    </div>
                </div>
        </form>
    ...
    <script>
        (function () {
            ...
            if (!box || !input || !preview || !placeholder || !overlay) return;
